...
| Публичный IP инстанса | `198.13.187.233` |
| Публичный домен | `custdevai.198-13-187-233.nip.io` (wildcard-DNS nip.io, A-запись не нужна) |
...
`custdevai.198-13-187-233.nip.io` уже резолвится в `198.13.187.233` без
ручной A-записи. Проверка с локальной машины:

```bash
dig +short custdevai.198-13-187-233.nip.io
# должно вернуть 198.13.187.233
```

Если в будущем переключаемся на собственный домен — создаём A-запись:

```
custdevai.198-13-187-233.nip.io  →  198.13.187.233
```
...
ssh -i ~/.ssh/selectel_ed25519 root@198.13.187.233
...
```bash
ssh -i ~/.ssh/selectel_ed25519 custdev@198.13.187.233
cd ~
...
PUBLIC_DOMAIN=custdevai.198-13-187-233.nip.io
...
TELEGRAM_WEBHOOK_URL=https://custdevai.198-13-187-233.nip.io/api/v1/telegram/webhook
TELEGRAM_WEBHOOK_SECRET=$(openssl rand -hex 32)

# Web base URL (Phase 4: используется во втором push).
WEB_BASE_URL=https://custdevai.198-13-187-233.nip.io
CORS_ALLOW_ORIGINS=["https://custdevai.198-13-187-233.nip.io"]
...
sudo certbot certonly --standalone \
    ...
    -d custdevai.198-13-187-233.nip.io

# 2. Создаём symlinks для docker монтирования.
mkdir -p docker/nginx/certs
sudo cp /etc/letsencrypt/live/custdevai.198-13-187-233.nip.io/fullchain.pem docker/nginx/certs/
sudo cp /etc/letsencrypt/live/custdevai.198-13-187-233.nip.io/privkey.pem docker/nginx/certs/
...
docker compose -f docker-compose.yml -f docker-compose.prod.yml exec api \
    ...
    --email admin@custdevai.198-13-187-233.nip.io \
...
curl -fsS -X POST \
    ...
    -F "url=https://custdevai.198-13-187-233.nip.io/api/v1/telegram/webhook" \
...
curl -fsS https://custdevai.198-13-187-233.nip.io/health
# {"status":"ok"}

# 2. Логин админа через cookie auth.
curl -fsS -c cookies.txt \
    -X POST "https://custdevai.198-13-187-233.nip.io/api/v1/auth/login?set_cookie=true" \
    -H "Content-Type: application/json" \
    -d '{"email":"admin@custdevai.198-13-187-233.nip.io","password":"..."}'

# 3. /me возвращает профиль.
curl -fsS -b cookies.txt https://custdevai.198-13-187-233.nip.io/api/v1/users/me

# 4. Откройте SPA в браузере: https://custdevai.198-13-187-233.nip.io
